      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {bookings.map((b, i) => {
            ...
            const canPay =
              showActions &&
              ["Confirmed", "PendingPayment"].includes(b.status) &&
              ps === "Unpaid";
            return (
              <tr
                key={b._id || i}
                    ...
                    >
                      {canPay && (
                        <>
                          ...
                          <button
                            disabled={loadingId === cashKey}
                            onClick={() => handleCollectEsewa(b._id)}
                            style={{
                              display: "flex",
                              alignItems: "center",
                              gap: 5,
...
function OverviewPanel({
  ...
  const unreviewedCount = conditionReports.filter(
    (r) => !r.conditionReportReviewed,
  ).length;
  const stats =
    role === "STAFF"
      ? [
          {
            ...
            subtitle: `${pendingDocsCount} pending`,
            icon: <FaFileAlt />,
            accent: "#f59e0b",
          },
          {
            ...
            subtitle: `${unreviewedCount} awaiting`,
            icon: <FaExclamationTriangle />,
            accent: "#ef4444",
          },
        ]
      ...
      {role === "STAFF" && (
        ...
        >
            ...
            <EmptyState
              ...
              label={
                pendingDocsCount === 0
                  ...
                  : `${pendingDocsCount} document${pendingDocsCount > 1 ? "s" : ""} to review`
              }
          ...
          <div style={card}>
            <PanelHeader
              title="Condition Reports"
              subtitle="Pre and post trip condition reports"
            />
            <EmptyState
              icon={<FaExclamationTriangle />}
              label={
                unreviewedCount === 0
                  ? "No open condition reports"
                  : `${unreviewedCount} report${unreviewedCount > 1 ? "s" : ""} awaiting review`
              }
...
function DriversPanel({ isAdmin }) {
  ...
  const saveRate = async (driverId) => {
    ...
    try {
      await axios.patch(
        `${ENDPOINTS.DRIVERS}/${driverId}/admin-update`,
  ...
  const saveDistrict = async (driverId) => {
    ...
    if (!["Kathmandu", "Lalitpur", "Bhaktapur"].includes(dist)) {
    ...
    try {
      ...
      alert(e.response?.data?.message || "Failed to save district.");
    } finally {
                      ...
                      {isAdmin ? (
                        editingDistrict?.id === dId ? (
                          ...
                          >
                            ...
                            <button
                              onClick={() => saveDistrict(dId)}
                              disabled={savingDistrict === dId}
                              style={{
                                padding: "5px 10px",
                                border: "none",
                                borderRadius: 6,
                          ...
                          <button
                            onClick={() =>
                              setEditingDistrict({
                                id: dId,
                                value: d.district || "Kathmandu",
                              })
                            }
                            style={{
                              fontSize: 12,
                              fontWeight: 600,
                              color: d.district ? "#0f172a" : "#94a3b8",
                              background: "none",
                              border: "none",
                              cursor: "pointer",
                              padding: "4px 8px",
...
function FinesPanel() {
  ...
  const filtered = fined.filter((b) => {
    ...
    const matchFilter =
      filter === "all" ? true : filter === "paid" ? b.finePaid : !b.finePaid;
    ...
    <div>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: "#64748b", margin: "2px 0 0" }}>
            {totalFined} booking{totalFined !== 1 ? "s" : ""} with fines
          </p>
        </div>
        <div
          style={{
      ...
      >
        {[
          ...
          {
            label: "Total fined",
            value: `Rs ${totalAmt.toLocaleString()}`,
            sub: "all fines",
            accent: "#F97316",
          },
          {
            label: "Collected",
            value: `Rs ${paidAmt.toLocaleString()}`,
            sub: "paid fines",
            accent: "#22C55E",
          },
          {
            label: "Outstanding",
            value: `Rs ${(totalAmt - paidAmt).toLocaleString()}`,
            sub: "unpaid",
            accent: "#dc2626",
          },
        ].map((s) => (
          ...
          return (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                cursor: "pointer",
                border: `1px solid ${active ? "#F97316" : f.alert ? "#fca5a5" : "#e2e8f0"}`,
                background: active ? "#F97316" : f.alert ? "#fef2f2" : "#fff",
                color: active ? "#fff" : f.alert ? "#dc2626" : "#64748b",
              }}
            >
              {f.label}
            </button>
        ...
        <div style={{ ...card, overflowX: "auto", padding: 0 }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {filtered.map((b, i) => {
                const daysExceeded =
                  b.returnedAt && b.endDate
                    ? Math.max(
                        0,
                        Math.ceil(
                          (new Date(b.returnedAt) - new Date(b.endDate)) /
                            (1000 * 60 * 60 * 24),
                        ),
                      )
                    : "—";
                  ...
                  >
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      ...
                      {b.vehicleFine > 0 && b.driverFine > 0 && (
                        ...
                        >
                          Vehicle: Rs {b.vehicleFine.toLocaleString()} · Driver:
                        ...
                        <div>
                          ...
                          {b.finePaidAt && (
                            <div style={{ fontSize: 10, color: "#94a3b8" }}>
                              {new Date(b.finePaidAt).toLocaleDateString(
                                "en-GB",
                              )}
                            </div>
                          )}
      ...
      >
        ...
        >
          <span style={{ color: "#e2e8f0", fontSize: 13, fontWeight: 600 }}>
            {label} · {SLOTS[idx] || `Photo ${idx + 1}`} ({idx + 1}/{total})
          </span>
          <button
            onClick={onClose}
            style={{
...
function ConditionReportsPanel({ reports, fetchReports }) {
  ...
  const openLightbox = (photos, index, label) =>
    setLightbox({ photos, index, label });

  const counts = {
    all: reports.length,
    preOnly: reports.filter(
      (r) => r.preTrip?.submittedAt && !r.postTrip?.submittedAt,
    ).length,
    postOnly: reports.filter(
      (r) => !r.preTrip?.submittedAt && r.postTrip?.submittedAt,
    ).length,
    complete: reports.filter(
      (r) => r.preTrip?.submittedAt && r.postTrip?.submittedAt,
    ).length,
    damage: reports.filter((r) => r.damageFlagged).length,
    unreviewed: reports.filter((r) => !r.conditionReportReviewed).length,
  };

  const filtered = reports.filter((r) => {
    if (filter === "damage") return r.damageFlagged;
    if (filter === "unreviewed") return !r.conditionReportReviewed;
    if (filter === "complete")
      return r.preTrip?.submittedAt && r.postTrip?.submittedAt;
    return true;
  });
  ...
  const toggleReview = async (id, current) => {
    ...
    try {
      await axios.patch(
        `${BASE_URL}/api/condition-reports/${id}/review`,
        { reviewed: !current },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      fetchReports();
  ...
  const toggleFlag = async (id, current) => {
    ...
    try {
      await axios.patch(
        `${BASE_URL}/api/condition-reports/${id}/review`,
        { damageFlagged: !current, damageFlaggedBy: !current ? "staff" : null },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      fetchReports();
    } catch {
  ...
  const TABS = [
    { key: "all", label: `All (${counts.all})` },
    {
      ...
      alert: counts.unreviewed > 0,
    },
    {
      key: "damage",
      label: `Damage flagged (${counts.damage})`,
      alert: counts.damage > 0,
    },
    { key: "complete", label: `Both reports (${counts.complete})` },
  ];
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {[
            {
              label: "Pickup only",
              count: counts.preOnly,
              color: "#1d4ed8",
              bg: "#eff6ff",
            },
            {
              label: "Return only",
              count: counts.postOnly,
              color: "#c2410c",
              bg: "#fff7ed",
            },
            {
          ...
          return (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                cursor: "pointer",
                border: `1px solid ${active ? "#F97316" : t.alert ? "#fca5a5" : "#e2e8f0"}`,
                background: active ? "#F97316" : t.alert ? "#fef2f2" : "#fff",
                color: active ? "#fff" : t.alert ? "#dc2626" : "#64748b",
              }}
            >
              {t.label}
            </button>
      ...
      {filtered.length === 0 ? (
        <div style={card}>
          <div style={{ padding: "48px 24px", textAlign: "center" }}>
            ...
            <p style={{ color: "#94a3b8", fontSize: 13, marginTop: 6 }}>
              Condition reports are optional — customers submit them before and
              after a trip.
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {filtered.map((r) => {
            ...
            const hasPreTrip = !!(
              r.preTrip?.submittedAt && r.preTrip?.photos?.length
            );
            const hasPostTrip = !!(
              r.postTrip?.submittedAt && r.postTrip?.photos?.length
            );
            const preDateStr = r.preTrip?.submittedAt
              ? new Date(r.preTrip.submittedAt).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })
              : null;
            const postDateStr = r.postTrip?.submittedAt
              ? new Date(r.postTrip.submittedAt).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })
            ...
            return (
              <div
                key={r._id}
                style={{
                  background: isReviewed ? "#f8fafc" : "#fff",
                  borderRadius: 14,
                  border: `1px solid ${r.damageFlagged ? "#fca5a5" : isReviewed ? "#f1f5f9" : "#e2e8f0"}`,
                  overflow: "hidden",
                ...
                >
                    ...
                    >
                      <span
                        style={{
                          fontSize: 14,
                          fontWeight: 700,
                          color: isReviewed ? "#64748b" : "#0f172a",
                        }}
                      >
                      ...
                      <ReportTypeBadge
                        hasPreTrip={!!r.preTrip?.submittedAt}
                        hasPostTrip={!!r.postTrip?.submittedAt}
                      />
                      {r.damageFlagged && (
                        <span
                          style={{
                        ...
                        >
                          ⚠ Damage flagged
                          {r.damageFlaggedBy ? ` by ${r.damageFlaggedBy}` : ""}
                        </span>
                      )}
                      {isReviewed && (
                        <span
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      ...
                      >
                        ...
                        {r.preTrip?.submittedAt ? (
                          ...
                          >
                            {hasPreTrip
                              ? `${r.preTrip.photos.length} photos · ${preDateStr}`
                              : `Submitted ${preDateStr} (no photos)`}
                          </span>
                        ) : (
                          <span
                            style={{
                      ...
                      >
                        ...
                        {r.postTrip?.submittedAt ? (
                          ...
                          >
                            {hasPostTrip
                              ? `${r.postTrip.photos.length} photos · ${postDateStr}`
                              : `Submitted ${postDateStr} (no photos)`}
                          </span>
                        ) : (
                          <span
                            style={{
                  ...
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    <button
                      ...
                      onClick={() => toggleFlag(r._id, r.damageFlagged)}
                      style={{
                        padding: "6px 12px",
                        border: `1px solid ${r.damageFlagged ? "#fca5a5" : "#e2e8f0"}`,
                        borderRadius: 8,
                        fontSize: 12,
                        fontWeight: 700,
                        color: r.damageFlagged ? "#dc2626" : "#64748b",
                        background: r.damageFlagged ? "#fef2f2" : "#fff",
                        cursor: flagging === r._id ? "not-allowed" : "pointer",
                        whiteSpace: "nowrap",
                      }}
                    >
                      {flagging === r._id
                    ...
                    <button
                      ...
                      onClick={() => toggleReview(r._id, isReviewed)}
                      style={{
                        padding: "6px 14px",
                        borderRadius: 8,
                        fontSize: 12,
                        fontWeight: 700,
                        cursor: toggling === r._id ? "not-allowed" : "pointer",
                        border: `1px solid ${isReviewed ? "#e2e8f0" : "#86efac"}`,
                        color: isReviewed ? "#64748b" : "#15803d",
                        background: isReviewed ? "#f1f5f9" : "#f0fdf4",
                        whiteSpace: "nowrap",
                      }}
                    >
                      {toggling === r._id
...
function DocumentsPanel() {
  ...
  const fetchPending = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${BASE_URL}/api/users/documents/pending`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const handleReview = async (id, action) => {
    ...
    try {
      await axios.patch(
        `${BASE_URL}/api/users/${id}/documents/review`,
        { action, reason },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      fetchPending();
    ...
    <div>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: "13px", color: "#64748b", margin: "2px 0 0" }}>
            {users.length} users waiting for review
          </p>
        </div>
      </div>
      {users.length === 0 ? (
        <div style={card}>
          <EmptyState
            ...
            hint="All good! There are no users waiting for document verification."
          />
        </div>
      ) : (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  <button
                    ...
                    onClick={() => handleReview(u._id, "reject")}
                    style={{
                      padding: "8px 16px",
                      border: "1px solid #fca5a5",
                      borderRadius: "8px",
                      ...
                      color: "#dc2626",
                      background: "#fff",
                      cursor: processing === u._id ? "not-allowed" : "pointer",
                    }}
                  >
                  ...
                  <button
                    disabled={processing === u._id}
                    onClick={() => handleReview(u._id, "approve")}
                    style={{
                      padding: "8px 16px",
                      border: "none",
                      borderRadius: "8px",
...
export default function Management() {
  ...
  const fetchPendingDocs = () => {
    axios
      .get(`${BASE_URL}/api/users/documents/pending`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((r) =>
        setPendingDocsCount(Array.isArray(r.data) ? r.data.length : 0),
      )
      .catch(() => {});
  };
  ...
  const fetchConditionReports = () => {
    axios
      .get(`${BASE_URL}/api/condition-reports`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((r) => setConditionReports(Array.isArray(r.data) ? r.data : []))
      .catch(() => {});
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "condition-reports":
        return (
          <ConditionReportsPanel
            reports={conditionReports}
            fetchReports={fetchConditionReports}
          />
        );
      case "staff":
        return (
