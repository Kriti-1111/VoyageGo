...
// Reusing pricing calc from BookingPage to show live prices
function calcPrice(
  pricePerHour,
  startDate,
  endDate,
...
export default function WalkInBookingModal({ onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    const fetchInitial = async () => {
      try {
        const vRes = await axios.get(`${API}/api/vehicles`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setVehicles(
          Array.isArray(vRes.data) ? vRes.data.filter((v) => v.isActive) : [],
        );
      } catch (err) {
        console.error(err);
      } finally {
    ...
    try {
      ...
      if (docsVerified && citizenshipFront && citizenshipBack) {
        try {
          ...
          await axios.post(
            `${API}/api/users/documents/walkin`,
            {
          ...
          console.error(
            "Document upload failed, but booking succeeded:",
            docErr,
          );
      ...
      if (paymentMethod === "eSewa") {
        ...
        alert(
          "Walk-in booking created and confirmed with Demo Payment (simulating eSewa)! 🎉",
        );
        onSuccess?.();
        onClose();
      } else {
        await axios.post(
          `${API}/api/pay/walkin/cash`,
          {
            bookingId: newBooking._id || newBooking.id,
          },
          { headers: { Authorization: `Bearer ${token}` } },
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
          {loadingInitial ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div style={{ gridColumn: "1 / -1", display: "flex", gap: 10 }}>
                  <button
                    ...
                    style={{
                      ...
                      padding: "8px",
                      borderRadius: 8,
                      border: `1.5px solid ${mode === "hourly" ? "#F97316" : "#e2e8f0"}`,
                      background: mode === "hourly" ? "#FFF7ED" : "#f8fafc",
                      color: mode === "hourly" ? "#F97316" : "#64748b",
                      fontWeight: 700,
                      cursor: "pointer",
                    }}
                  >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: "8px",
                      borderRadius: 8,
                      border: `1.5px solid ${mode === "daily" ? "#F97316" : "#e2e8f0"}`,
                      background: mode === "daily" ? "#FFF7ED" : "#f8fafc",
                      color: mode === "daily" ? "#F97316" : "#64748b",
                      fontWeight: 700,
                      cursor: "pointer",
                    }}
                  >
              ...
              >
                <label
                  style={{
                    ...
                    fontWeight: 700,
                    color: "#0f172a",
                    marginBottom: requiresDriver ? 12 : 0,
                    cursor: "pointer",
                  }}
                >
                  <input
                ...
                {requiresDriver && (
                  <div>
                    <label style={lbl}>
                      Prefer Specific Driver? (Optional)
                    </label>
                    <select
                      style={inp}
                      value={driverId}
                    ...
                    >
                      <option value="">
                        -- Any Available / Auto-Assign --
                      </option>
                      {driverPool.map((d) => (
                        <option key={d._id || d.id} value={d._id || d.id}>
                          {d.name} (Rs {d.driverRatePerHour}/hr)
                        </option>
                      ))}
                    </select>
                  </div>
              ...
              {/* CUSTOMER DOCUMENTS (InPerson Verification) */}
              <div
                style={{
                  background: "#f8fafc",
                  padding: 16,
              ...
              >
                ...
                <label
                  style={{
                    ...
                    fontSize: 13,
                    fontWeight: 700,
                    color: docsVerified ? "#15803d" : "#0f172a",
                    cursor: "pointer",
                  }}
                >
                  <input
              ...
              {priceEstimate && priceEstimate.total !== null && (
                ...
                >
                  ...
                  <span style={{ fontSize: 16 }}>
                    Rs {priceEstimate.total.toLocaleString()}
                  </span>
                </div>
              )}
            </div>
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              color: "#fff",
              fontWeight: 600,
              cursor: submitting || loadingInitial ? "not-allowed" : "pointer",
              opacity: submitting || loadingInitial ? 0.7 : 1,
            }}
          >
